...
class DojangAddressAnalyzerTest {

    private DojangAddressAnalyzer analyzer;

    @BeforeEach
    void setUp() {
        analyzer = new DojangAddressAnalyzer();
    }

    @Test
    @DisplayName("기본 주소 토큰화")
    void basicAddress() {
        Set<String> tokens = analyzer.analyze("서울 강남구 논현동");
    ...
    void extractDongInParenthesis() {
        Set<String> tokens = analyzer.analyze("서울 강남구 강남대로122길 39-9 202호(논현동)");
    ...
    void guTokenization() {
        Set<String> tokens = analyzer.analyze("서울 강남구");
    ...
    void dongPattern() {
        Set<String> tokens = analyzer.analyze("경기 화성시 향남읍 행정동로");
    ...
    void complexAddress() {
        Set<String> tokens = analyzer.analyze("경기 광명시 하안동 광명e-편한세상동부센츠레빌 단지내 상가");
    ...
    void chosungGeneration() {
        Set<String> tokens = analyzer.analyze("대치동");
